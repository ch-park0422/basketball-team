...
export default function UsersTable({ initialMembers, currentUserId }: Props) {
  ...
  return (
    <>
      <p className="text-[#6e6e73] text-[14px] mt-1 mb-8">
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead className="border-b border-black/[0.06]">
            <tr>
              ...
              <th className="text-left px-5 py-3.5 text-[11px] font-semibold text-[#6e6e73] uppercase tracking-widest">관리</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-black/[0.04]">
            {members.map((profile, i) => (
              <tr
                key={profile.id}
                className={`hover:bg-black/[0.02] transition-colors ${profile.id === currentUserId ? "bg-black/[0.02]" : ""}`}
              >
                ...
                <td className="px-5 py-4">
                  {profile.id !== currentUserId ? (
                    ...
                    <span className="text-[12px] text-[#c7c7cc]">삭제 불가</span>
                  )}
                </td>
              </tr>
            ))}
